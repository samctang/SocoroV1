using System;
using System.Collections.Generic;
using Socoro.Model.Core;

namespace Socoro.Data.Core
{
    public class UnitOfWork : IUnitOfWork
    {
        public BaseContext Context { get; set; }
        internal Dictionary<Type, object> RepositoryCache { get; private set; }
        public UnitOfWork(BaseContext dbContext)
        {
            RepositoryCache = new Dictionary<Type, object>();
            Context = dbContext;
        }
        public void Commit()
        {
            Context.SaveChanges();
        }

        public virtual void Dispose(bool disposing)
        {
            if(disposing && Context != null)
            {
                Context.Dispose();
            }
        }
        public void Dispose()
        {
            Dispose(true);
            GC.SuppressFinalize(this);
        }
        public IRepository<T> GetEntityRepository<T>() where T : class, IIdentifier
        {
            object repoObj;
            if(RepositoryCache.TryGetValue(typeof(IRepository<T>), out repoObj))
            {
                return (IRepository<T>)repoObj;
            }
            else
            {
                IRepository<T> repo = new Repository<T>(Context);
                RepositoryCache[typeof(IRepository<T>)] = repo;
                return repo;
            }
        }
        public R GetCustomRepository<R>() where R : class
        {
            object repoObj;
            if(RepositoryCache.TryGetValue(typeof(R), out repoObj)){
                return (R)repoObj;
            }else
            {
                throw new NotImplementedException("No repo registered for type " + typeof(R) + " in UnitOfWork");
            }
        }
        public void RegisterEntityRepository<E>(IRepository<E> repository) where E : class, IIdentifier
        {
            RepositoryCache[typeof(IRepository<E>)] = repository;
        }
        public void RegisterCustomRepository<R>(R repository) where R : class
        {
            RepositoryCache.Add(typeof(R), repository);
        }
    }
}
